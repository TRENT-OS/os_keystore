 ...
 * instantiation of the KeystoreFile), which allows having several instances of
 * the KeystoreFile on the same file system. Each key is stored in its own file
 * and the file name is constructed from a Keystore's "instance name" and the
 * respective "key name".
 *
 * Info: Using different instances of the KeystoreFile with the same file system
 * requires each instance to have a unique instance name. Otherwise, these
 * instances might interfere with each other's files.
 *
 * Info: The isolation between two KeystoreFile instances using the same piece
 ...
 * storage (disjunctive ranges of storage can be assigned via the
 * StorageServer).
 */
 ...
 *                      to be used to store the keys when using the API
